...
const print= (x)=>console.log(JSON.stringify(x,null,3));
...
export type CompoundTExp = ProcTExp | TupleTExp |ourPairTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isOurPairTExp(x);

export type NonTupleTExp = AtomicTExp | ProcTExp | TVar |TExp; 
...
//our code: ourPairTExp,isOurPairTExp
export type ourPairTExp = { tag: "ourPairTExp", temp1: TExp, temp2: TExp };

export const ourMakePairTExp = (temp1: NonTupleTExp, temp2: NonTupleTExp): ourPairTExp => 
    ({tag: "ourPairTExp", temp1: temp1, temp2: temp2});

export const isOurPairTExp = (x: any): x is ourPairTExp => x.tag === "ourPairTExp";

export const PairTExpComponents = (pt: ourPairTExp): TExp[] => [pt.temp1, pt.temp2];
//end
...
*/
const parseCompoundTExp = (texps: any[]): ourPairTExp | ProcTExp | Error => {
    const pos = texps.indexOf('->');
    return  (texps.length == 3)&&(texps[0]=="Pair")? ourSafeMakePairTExp(parseTExp(texps[1]),parseTExp(texps[2])):
    ...
    safeMakeProcTExp(parseTupleTExp(texps.slice(0, pos)),parseTExp(texps[pos + 1]));
};

//our function:
const ourSafeMakePairTExp = (temp1: TExp | Error, temp2: TExp | Error): Error | ourPairTExp =>
    isError(temp1) ? temp1 :
    isError(temp2) ? temp2 :
    ourMakePairTExp(temp1, temp2); 
//end
...
export const unparseTExp = (te: TExp | Error): string | Error => {
    ...
    const up = (x: TExp | Error): string | string[] | Error =>{
        ...
        isOurPairTExp(x) ? `(Pair ${unparseTExp(x.temp1)} ${unparseTExp(x.temp2)})` :
        ["never"];
    }
...
export const matchTVarsInTE = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isAtomicTExp(te1) || isAtomicTExp(te2)) ?
        ...
        isOurPairTExp(te1) && isOurPairTExp(te2)? matchTVarInPair(te1,te2,succ,fail):



    matchTVarsInTProcs(te1, te2, succ, fail);

    const matchTVarInPair = <T1,T2>(te1: TExp, te2: TExp,
        succ: (mapping: Array<Pair<TVar, TVar>>) => T1,
        fail: () => T2): T1 | T2=>{
    
        if (isOurPairTExp(te1) && isOurPairTExp(te2)){
            const ourTemp1 = [te1.temp1,te1.temp2]
            const ourTemp2 = [te2.temp1,te2.temp2]
            return matchTVarsInTEs(ourTemp1,ourTemp2,succ,fail);
        } 
        else
            return fail()
