...
export const makeEquationFromExp = (exp: A.Exp, pool: Pool): Equation[] =>
    ...
    //our implement
    A.isOurQuote(exp) ? [makeEquation(inPool(pool, exp), T.ourMakePairTExp(inPool(pool,exp.left),inPool(pool,exp.right)))]:
    //end
...
export const canUnify = (eq: Equation): boolean =>
    T.isOurPairTExp(eq.left) && T.isOurPairTExp(eq.right) ? true:
...
export const splitEquation = (eq: Equation): Equation[] =>
    T.isOurPairTExp(eq.left)&&T.isOurPairTExp(eq.right) ? [makeEquation(eq.left.temp1,eq.right.temp1),makeEquation(eq.left.temp2,eq.right.temp2)]:
    (T.isProcTExp(eq.left) && T.isProcTExp(eq.right)) ?
