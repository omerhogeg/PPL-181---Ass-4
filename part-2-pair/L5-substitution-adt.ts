...
import { eqTVar, isAtomicTExp, isProcTExp, isTVar, makeProcTExp, unparseTExp, TExp, TVar,NonTupleTExp ,isOurPairTExp, ourMakePairTExp } from "./TExp";
...
export const checkNoOccurrence = (tvar: TVar, te: TExp): true | Error => {
    const check = (e: TExp): true | Error =>{
        return isTVar(e) ? ((e.var === tvar.var) ? Error(`Occur check error - circular sub ${tvar.var} in ${unparseTExp(te)}`) : true) :
        isAtomicTExp(e) ? true :
        isOurPairTExp(e) ? (isError(check(e.temp1))? Error(`Error: in type expression ${e.temp1} in ${te}`): isError(check(e.temp2))? Error(`Error: in type expression ${e.temp1} in ${te}`):true): 
...
export const applySub = (sub: Sub, te: TExp): TExp =>
    ...
    isOurPairTExp(te)? ourMakePairTExp(<NonTupleTExp>applySub(sub,te.temp1),<NonTupleTExp>applySub(sub,te.temp2)) :
