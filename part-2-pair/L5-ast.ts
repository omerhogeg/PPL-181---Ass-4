...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp,isOurPairTExp } from './TExp';
...
//our code:
export type ourQuote = {tag: "ourQuote",left: CExp,right: CExp};
export const toMakeQuote = (left:CExp,right: CExp):ourQuote=>({tag:"ourQuote",left: left,right: right})
export const isOurQuote = (x:any):x is ourQuote=> x.tag==="ourQuote";
//end
...
export type CompoundExp = AppExp | IfExp | ProcExp | LetExp | LitExp | LetrecExp | SetExp | ourQuote;
export const isCompoundExp = (x: any): x is CompoundExp =>
    isAppExp(x) || isIfExp(x) || isProcExp(x) || isLitExp(x) || isLetExp(x) || isLetrecExp(x) || isSetExp(x) ||isOurQuote(x);
export const expComponents = (e: Exp): CExp[] =>
    ...
    isSetExp(e) ? [e.val] :
    isDefineExp(e) ? [e.val] :
    isOurQuote(e) ? [e.left,e.right] : //we add it for isOurQuote
...
const parseCompound = (sexps: any[]): Parsed | Error =>
    ...
    first(sexps)==="quote"?parseForQuoteExp(sexps) :
    parseCExp(sexps);
...
//our function:
const parseForQuoteExp=(sexp:any[]):ourQuote|Error=>{
     let myLeft = parseCExp(second(sexp)[0]);
     let myRight = parseCExp(second(sexp)[2]);
     if(isError(myLeft) || isError(myRight)){
        return Error(`Error: the left or the right is bad!: ${sexp}`)
    }
    return second(sexp).length===3 && second(sexp)[1]==="."?
    toMakeQuote(myLeft,myRight):
    Error(`Error: in Quote sexp ${sexp} `);}
//end
...
export const parseLitExp = (sexps: any[]): ourQuote| LitExp | Error =>
    second(sexps).length===3 && second(sexps)[1]==="." ? parseForQuoteExp(sexps):
...
export const unparse = (e: Parsed | Error): string | Error =>
    ...
    isOurQuote(e) ? `'(${unparse(e.left)} . ${unparse(e.right)})` : //our!
