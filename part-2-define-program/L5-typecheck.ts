...
['+', '-', '*', '/'].some((x) => x === p.op) ? numOpTExp :
['and', 'or'].some((x) => x === p.op) ? boolOpTExp :
['>', '<', '='].some((x) => x === p.op) ? numCompTExp :
['number?', 'boolean?', 'string?', 'symbol?', 'list?'].some((x) => x === p.op) ? typePredTExp :
...
export const typeofDefine = (exp: DefineExp, tenv: TEnv): TExp | Error => {
    // return Error("TODO");
    const constraint = checkEqualType(exp.var.texp, typeofExp(exp.val, tenv), exp);
...
// then compute the rest of the exps in the new env.[we did it like in L1-eval.ts]
export const typeofProgram = (exp: Program, tenv: TEnv): TExp | Error => {
    //return Error("TODO");
    if(isEmpty(exp.exps)) {
        return Error("ERROR:empty program");
    }
    if(isError(typeofExp(first(exp.exps), tenv))){
        return typeofExp(first(exp.exps), tenv);
    }
    if(isDefineExp(first(exp.exps))){
        if(isEmpty(rest(exp.exps))){
            return makeVoidTExp();
        }
        else{
           return typeofProgram(makeProgram(rest(exp.exps)), makeExtendTEnv([first(exp.exps).var.var], [first(exp.exps).var.texp], tenv));
        }
    }
    if (isEmpty(rest(exp.exps))){
        return typeofExp(first(exp.exps), tenv);
    }
    return typeofProgram(makeProgram(rest(exp.exps)), tenv);
};
